...
%   algoritmanin durumu ve emir anlari.
    ...
    t = 0; i = 0; takla_yapildi = false;
    sonraki_sensor = 0;

    while t < p.t_bitis
        % ---------- SENSOR + ALGORITMA (100 Hz) ----------
        if t >= sonraki_sensor - 1e-9
            sonraki_sensor = sonraki_sensor + p.dt_sensor;
            i = i + 1;
            ...
            % Egim [0,180] araligina SARILARAK kaydedilir. abs(theta)
            % kullanmak takla halinde 180 dereceyi asan anlamsiz degerler
            % uretir (theta unwrapped birikir) ve kisit kontrolunu bozar.
            s.tilt(i)      = acos(cos(y(5)))*180/pi;
        ...
        % ---------- FIZIK (RK4, degisken adim) ----------
        % Tirmanis ve apogee civari hizli dinamik -> kucuk adim.
        % Parasut altinda terminal hiza oturulur, dinamik yavastir -> buyuk
        % adim yeterli. Inis ucus suresinin ~%90'i oldugu icin bu, kosum
        % suresini bir kat mertebesinde kisaltir.
        if ~isnan(drogue_t) && t > drogue_t + 3
            h = p.dt_fizik_inis;
        else
            h = p.dt_fizik;
        end
        h = min(h, max(sonraki_sensor - t, p.dt_fizik));

        y = rk4(y, t, h, p, drogue_t, ana_t, ariza);
...
%   Algoritmanin gordugu tek sey budur — gercek durumu asla gormez.
    ...
    g.z = z;  g.tilt = acos(cos(th))*180/pi;
    ...
    % --- Yonelim ---
    % Egim duzeltmesi: modelin apogee civarindaki egim eksikligini telafi
    % eder (gerekcesi roket_params.m'de). Yalniz ALGORITMAYA giden olcumu
    % etkiler; fizik gercek th ile devam eder.
    % ONCE SAR, SONRA OLCEKLE, SONRA DOYUR.
    % Ham (sarilmamis) aciyi olceklemek takla halinde felakettir: roket
    % birkac tur donunce ornegin 1185 derece x 2.09 = 2477 -> sarilinca 43
    % derece cikar ve guvenlik kapisi YANLISLIKLA ACILIR. Olcekleme yalniz
    % fiziksel egim acisi (0-180) uzerinde anlamlidir ve 180'de doyurulur.
    th_sarili = atan2(sin(th), cos(th));                  % [-pi, pi]
    th_ol = sign(th_sarili) * min(abs(th_sarili)*p.egim_duzeltme, pi);
    th_ol = th_ol + (p.aci_sigma*pi/180) * randn();
...
%KUVVETLER  Toplam kuvvet [x z] ve yunuslama momenti.
    ...
    % V esigi: cok dusuk hizda atan2 ile hesaplanan hucum acisi anlamsiz
    % buyur. Rampa fazi ayrica ele alindigi icin esik dusuk tutulabilir;
    % apogee civarinda weathercocking'in devam etmesi SART (OpenRocket
    % referansi apogee-de 24.9 derece egim gosteriyor).
    kararli = isnan(ariza.kararlilik_kaybi_t) || t < ariza.kararlilik_kaybi_t;
    if V > p.aero_moment_min_hiz && isnan(drogue_t) && kararli
...
function ydot = tureva(y, t, p, drogue_t, ana_t, ariza)
    ...
    J = p.J;                            % .ork'tan: boyuna atalet momenti
